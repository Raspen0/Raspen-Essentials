...
public class CommandHeal implements CommandCallable {
    ...
    public CommandResult process(CommandSource src, String arguments) throws CommandException {
        ...
        if(player == null){
            ...
            src.sendMessage(Text.of(plugin.getManager().getLangHandler().getMessage(src, null, "notOnline")));
            return CommandResult.success();
        }
        ...
        src.sendMessage(Text.of(plugin.getManager().getLangHandler().getMessage(src, null, "healed")));
        if (player != src) {
           // src.sendMessage(Text.of(plugin.getManager().getLangHandler().getMessage(src, null, "healedOther").replace("%player", args[0])));
            src.sendMessage(Text.of(plugin.getManager().getLangHandler().getMessage(src, null, "healedOther")));
        }
        return CommandResult.success();
    }
